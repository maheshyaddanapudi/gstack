...
Map `_NEW_LEVEL` to the snooze duration: level 1 = 24h, level 2 = 48h, level 3 = 1 week. Tell user the snooze duration: "Next reminder in 24h" (or 48h or 1 week). Tip: "Set `auto_upgrade: true` in `~/.gstack/config.yaml` for automatic upgrades."
...
```bash
cd "$INSTALL_DIR"
cp -Rf "$INSTALL_DIR" "$INSTALL_DIR.bak"
...
```
If `./setup` fails, restore from backup: `rm -rf "$INSTALL_DIR" && mv "$INSTALL_DIR.bak" "$INSTALL_DIR"` and warn the user.
...
Read `$INSTALL_DIR/CHANGELOG.md`. Version entries are `## vX.Y.Z` headings. Scan from the top, collect all entries where the version is newer than `$OLD_VERSION`. For each, read the bullets below until the next `##` heading. Summarize the combined changes as 5-7 bullets grouped by theme. Don't overwhelm — focus on user-facing changes. Skip internal refactors unless they're significant.
