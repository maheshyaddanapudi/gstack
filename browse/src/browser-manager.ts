...
export class BrowserManager {
  ...
  async launch() {
    ...
    // user on Linux (crbug.com/638180). Detect all three cases and add
  ...
  async closeTab(id?: number): Promise<void> {
    ...
    // Note: page.close() fires the wirePageEvents 'close' handler, which may
    // have already reset activeTabId to 0 by the time we get here. So decide
    // what to do from `wasActive` (captured before the await) and the current
    // pages map — never from `this.activeTabId`, which the handler mutates.
    if (this.pages.size === 0) {
      // Closed the last tab — always auto-create a blank one so the browser
      // is never left with zero tabs (race-proof against the close handler).
      await this.newTab();
    } else if (wasActive) {
      // Closed the active tab but others remain — focus the most recent.
