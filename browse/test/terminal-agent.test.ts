...
describe('Source-level guard: terminal-agent', () => {
  ...
  test('lazy spawn: claude PTY is spawned in message handler, not on upgrade', () => {
    ...
    // The upgrade must not spawn the PTY under either the historical name
    // (spawnClaude) or the current lazy-spawn entry point (maybeSpawnPty).
    ...
    // The handler routes both the explicit `start` frame and the first raw
    // input byte through maybeSpawnPty(), which internally calls spawnClaude.
