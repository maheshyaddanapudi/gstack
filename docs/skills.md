...
| [`/autoresearch`](#autoresearch) | **Autonomous Researcher** | Karpathy's autoresearch loop at CPU scale — a remote LLM agent proposes a change, a fast objective measures it, the change is kept only if the metric improves. Optimizes a real ML training metric, or fans out researcher agents to keep improving a repo (test-verified fixes only). |
